...
```
The script produces:
- Raw review and metadata parquet files
- Merged data (reviews + metadata) where each review is a single row
- Document IDs (`parent_asin`) in a pickle file
- Product documents in a pickle file
- Merged product-level parquet file (one row per product)
